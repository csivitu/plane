...
export DOCKERHUB_USER=resterfarerize
...
function buildLocalImage() {
  ...
  if [ "$DO_BUILD" == "1" ] || [ "$DO_BUILD" == "" ]; then
    REPO=https://github.com/csivitu/plane.git
    CURR_DIR=$PWD
...
function download() {
  cd $SCRIPT_DIR
  TS=$(date +%s)
  if [ -f "$PLANE_INSTALL_DIR/docker-compose.yaml" ]; then
    mv $PLANE_INSTALL_DIR/docker-compose.yaml $PLANE_INSTALL_DIR/archive/$TS.docker-compose.yaml
  fi

  curl -H 'Cache-Control: no-cache, no-store' -s -o $PLANE_INSTALL_DIR/docker-compose.yaml https://raw.githubusercontent.com/csivitu/plane/$BRANCH/deploy/selfhost/docker-compose.yml?$(date +%s)
  curl -H 'Cache-Control: no-cache, no-store' -s -o $PLANE_INSTALL_DIR/variables-upgrade.env https://raw.githubusercontent.com/csivitu/plane/$BRANCH/deploy/selfhost/variables.env?$(date +%s)
...
function startServices() {
  ...
  local idx2=0
  while ! docker logs $api_container_id 2>&1 | grep -m 1 -i "Application startup complete" | grep -q "."; do
...
CPU_ARCH=$(uname -m)
if [[ $FORCE_CPU == "amd64" || $CPU_ARCH == "amd64" || $CPU_ARCH == "x86_64" || ($BRANCH == "master" && ($CPU_ARCH == "arm64" || $CPU_ARCH == "aarch64")) ]]; then
  USE_GLOBAL_IMAGES=1
  DOCKERHUB_USER=resterfarerize
  PULL_POLICY=always
else
  USE_GLOBAL_IMAGES=0
